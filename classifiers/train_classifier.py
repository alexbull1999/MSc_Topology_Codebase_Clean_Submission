...
def run_cross_validation(
    ...
    if not hyperparameter_search:
        ...
        logger.info(f"Label distribution: {torch.bincount(y).tolist()}")
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
        ...
        fold_results = train_single_fold(
            model, train_loader, val_loader, config, device, fold, hyperparameter_search
        )
        ...
        if save_models and not hyperparameter_search:
            ...
            torch.save({
                ...
                'fold_results': asdict(fold_results)
            }, model_path)
...
def main():
    ...
    parser.add_argument('--data_path', type=str,
                        default='results/tda_integration/neural_network_data_SNLI_1k.pt',
    ...
    try:
        ...
        training_data = load_neural_network_data(args.data_path)
        ...
        numeric_labels = np.array([label_to_idx[label] for label in training_data['labels']])
