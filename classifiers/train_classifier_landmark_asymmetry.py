"""
Lean Enhanced TDA Neural Classifier Training Pipeline

Simple adaptation of the original training script to handle 14D asymmetric features.
...
# Import the enhanced classes we already created
...
# Remove these redundant classes since we're importing the enhanced versions
# class FeatureNormalizer:
# class TDANeuralClassifier:
...
class TrainingConfig:
    """Enhanced config for 14D features with attention option"""
...
def train_single_fold(
        model: EnhancedTDANeuralClassifier,  # Use enhanced classifier
    ...
    """Minimal changes from original - just updated logging"""
...
def run_cross_validation(
    ...
    """Minimal changes from original - just removed feature_names parameter"""
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(features, labels)):
        ...
        normalizer = EnhancedFeatureNormalizer()  # Use enhanced normalizer

        X_train_raw, X_val_raw = features[train_idx], features[val_idx]
        y_train_np, y_val_np = labels[train_idx], labels[val_idx]

        X_train = normalizer.fit_transform(X_train_raw, [])  # Feature names not needed for training
        ...
        # Create enhanced model with correct input dimension and attention option
        model = EnhancedTDANeuralClassifier(
...
def summarize_cv_results(results: List[TrainingResults]) -> Dict:
    """Enhanced summary with baseline comparison"""
    ...
    # Print summary
    logger.info(f"Validation Accuracy: {summary['validation_accuracy']['mean']:.2f}% ± {summary['validation_accuracy']['std']:.2f}%")
    logger.info(f"Range: {summary['validation_accuracy']['min']:.2f}% - {summary['validation_accuracy']['max']:.2f}%")
    logger.info(f"Validation F1-Macro: {summary['validation_f1_macro']['mean']:.4f} ± {summary['validation_f1_macro']['std']:.4f}")
    ...
    # Enhanced success criteria with baseline comparison
    baseline_accuracy = 52.0
    improvement = avg_val_acc - baseline_accuracy
    
    logger.info(f"\nPERFORMANCE COMPARISON:")
    logger.info(f"Enhanced Accuracy: {avg_val_acc:.2f}%")
    logger.info(f"Original Baseline: {baseline_accuracy:.2f}%")
    logger.info(f"Improvement: +{improvement:.2f} percentage points")
    
    if avg_val_acc > 75:
        logger.info(f"EXCELLENT: Enhanced accuracy > 75%! Asymmetric features working great!")
    elif avg_val_acc > 65:
        logger.info(f"VERY GOOD: Enhanced accuracy > 65%! Clear improvement from 14D features!")
    elif avg_val_acc > 55:
        logger.info(f"GOOD: Enhanced accuracy > 55%! Modest improvement from enhanced features")
    elif avg_val_acc > 52:
        logger.warning(f"MODEST: Enhanced accuracy > baseline but < 55% - consider tuning")
    else:
        logger.error(f"POOR: Enhanced accuracy below 52% baseline - needs investigation")
