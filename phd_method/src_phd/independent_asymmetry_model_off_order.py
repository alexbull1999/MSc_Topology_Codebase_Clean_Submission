...
class AsymmetryTransformModel(nn.Module):
    ...
    def __init__(self, hidden_size: int = 768, order_model_path: str=None):
        ...
        # Load and freeze the order model FOR V4
        if order_model_path:
            print(f"Loading order model from: {order_model_path}")
            order_checkpoint = torch.load(order_model_path, map_location='cpu')
            self.order_model = OrderEmbeddingModel(hidden_size=hidden_size)
            self.order_model.load_state_dict(order_checkpoint['model_state_dict'])
            
            # Freeze order model parameters
            for param in self.order_model.parameters():
                param.requires_grad = False
            self.order_model.eval()
            print("✅ Order model loaded and frozen")
        else:
            self.order_model = None
            print("⚠️ No order model provided - will expect order embeddings as input")
    ...
    def forward(self, sbert_tokens: torch.Tensor) -> torch.Tensor:
        ...
        if self.order_model is not None:
            # Pass through frozen order model first
            with torch.no_grad():
                order_embeddings = self.order_model(sbert_tokens)
        else:
            # Assume input is already order embeddings
            order_embeddings = sbert_tokens
        
        # Apply asymmetric transformation to order embeddings
        asymmetric_features = self.asymmetric_transform(order_embeddings)
...
class AsymmetryTrainer:
    ...
    def compute_asymmetric_loss(self, premise_tokens: torch.Tensor, hypothesis_tokens: torch.Tensor,
        ...
        if label_str == 'entailment':
            ...
            asymmetric_loss += F.relu(torch.tensor(1.0, device=self.device) - asymmetric_energy)  # Encourage asymmetric > 0.8
            ...
            target_forward = torch.tensor(1.0, device=self.device)
            target_backward = torch.tensor(1.0, device=self.device)
            target_asymmetric = torch.tensor(0.3, device=self.device)
            asymmetric_loss += F.mse_loss(forward_energy, target_forward)
            asymmetric_loss += F.mse_loss(backward_energy, target_backward)
            asymmetric_loss += F.mse_loss(asymmetric_energy, target_asymmetric)
...
def train_asymmetry_model_only(processed_data_path: str, output_dir: str, order_model_path: str,
    ...
    asymmetry_model = AsymmetryTransformModel(hidden_size=768, order_model_path=order_model_path)
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            asymmetry_model_path = Path(output_dir) / "new_independent_asymmetry_transform_model_v4.pt"
            torch.save({
