...
# Import pure hyperbolic functions
from src.hyperbolic_order_embeddings_asymmetry import (
    PureHyperbolicLearnableOrderEmbeddingModel, 
    PureHyperbolicOrderEmbeddingTrainer,
    train_pure_hyperbolic_order_embeddings
)
from src.order_embeddings_asymmetry import EntailmentDataset
...
class HyperbolicLatticeEvaluator:
    ...
    def _create_hyperbolic_order_embedding_data(self, model, processed_data_path, sample_size=10000):
        """
        ...
        model.eval()
        with torch.no_grad():
            #Map BERT embeddings to hyperbolic space
            premise_hyperbolic = model(sampled_premise.to(self.device)).cpu().numpy()
            hypothesis_hyperbolic = model(sampled_hypothesis.to(self.device)).cpu().numpy()
    
        return premise_hyperbolic, hypothesis_hyperbolic, np.array(sampled_labels)


    def evaluate_hyperbolic_model_comprehensive(self, model, processed_data_path):
        """
        Use existing lattice metric implementations for comprehensive evaluation
        """
        # Create order embedding data
        premise_hyperbolic, hypothesis_hyperbolic, labels = self._create_hyperbolic_order_embedding_data(model, processed_data_path)
        ...
        results = self.lattice_analyzer.test_embedding_space(
            ...
            labels=labels,
            space_name="hyperbolic_order_embeddings"
        )
...
def comprehensive_hyperparameter_search():
    ...
    evaluator = HyperbolicLatticeEvaluator()
    results = []
    best_score_so_far = -1
    ...
    for i, (order_dim, asymmetry_weight, margin, lr) in enumerate(product(*search_params.values())):
        ...
        comprehensive_stats = evaluator.evaluate_hyperbolic_model_comprehensive(model, eval_data_path)
        ...
        if result['overall_score'] > best_score_so_far:
            ...
            save_best_checkpoint_and_model(best_result_so_far, model, trainer, 
                                            f"hyperbolic_best_config_lattice_OE")
...
def create_train_eval_split(processed_data_path, train_ratio=0.8, seed=42, max_samples=10000):
    ...
    # Save splits
    train_path = processed_data_path.replace('.pt', '_hyperbolic_train_OELattice_split.pt')
    eval_path = processed_data_path.replace('.pt', '_hyperbolic_eval_OELattice_split.pt')
...
def save_best_checkpoint_and_model(best_result, model, trainer, prefix):
    ...
    results_dir = Path("entailment_surfaces/results/hyperbolic_hyperparam_search_lattice_OE_model")
