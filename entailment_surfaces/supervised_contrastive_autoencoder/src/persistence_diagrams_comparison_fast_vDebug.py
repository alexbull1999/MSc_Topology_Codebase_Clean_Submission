...
def ph_dim_and_diagrams_from_distance_matrix(dm: np.ndarray,
    ...
    print(f"DEBUG: Starting persistence computation on {dm.shape[0]}x{dm.shape[0]} distance matrix")
    print(f"DEBUG: Distance matrix stats - min: {np.min(dm):.6f}, max: {np.max(dm):.6f}, mean: {np.mean(dm):.6f}")
    
    test_n = range(min_points, max_points, point_jump)
    lengths = []
    all_diagrams = []
    
    for i, points_number in enumerate(test_n):
        sample_indices = np.random.choice(dm.shape[0], points_number, replace=False)
        dist_matrix = dm[sample_indices, :][:, sample_indices]
        
        print(f"DEBUG: Sample {i+1}/{len(list(test_n))}, using {points_number} points")
        print(f"DEBUG: Subsampled distance matrix stats - min: {np.min(dist_matrix):.6f}, max: {np.max(dist_matrix):.6f}")
        ...
        print(f"DEBUG: Raw H0 diagram shape: {h0_diagram.shape}")
        print(f"DEBUG: Raw H1 diagram shape: {h1_diagram.shape}")
        
        # Check for infinite values in H0
        infinite_mask_h0 = np.isinf(h0_diagram).any(axis=1)
        finite_h0 = h0_diagram[~infinite_mask_h0]
        infinite_h0 = h0_diagram[infinite_mask_h0]
        
        print(f"DEBUG: H0 finite features: {len(finite_h0)}, infinite features: {len(infinite_h0)}")
        
        if len(finite_h0) > 0:
            h0_persistences = finite_h0[:, 1] - finite_h0[:, 0]
            print(f"DEBUG: H0 finite persistence stats - min: {np.min(h0_persistences):.6f}, max: {np.max(h0_persistences):.6f}, mean: {np.mean(h0_persistences):.6f}")
        
        if len(infinite_h0) > 0:
            print(f"DEBUG: H0 infinite births range: {np.min(infinite_h0[:, 0]):.6f} to {np.max(infinite_h0[:, 0]):.6f}")
        ...
        all_diagrams.append({
            ...
            'infinite_h0_count': len(infinite_h0)
        })
        
        # Compute persistence lengths for PH dimension calculation (filter out infinite)
        d = diagrams[h_dim]
        d = d[d[:, 1] < np.inf]
        lengths.append(np.power((d[:, 1] - d[:, 0]), alpha).sum())
    
    # Final analysis on largest sample
    final_diagrams = all_diagrams[-1]
    final_h0 = final_diagrams['H0']
    
    print(f"\nDEBUG: FINAL H0 ANALYSIS (n_points={max_points}):")
    print(f"DEBUG: Total H0 features: {len(final_h0)}")
    print(f"DEBUG: Finite H0 features: {final_diagrams['finite_h0_count']}")
    print(f"DEBUG: Infinite H0 features: {final_diagrams['infinite_h0_count']}")
    
    # Detailed breakdown
    infinite_mask = np.isinf(final_h0).any(axis=1)
    finite_features = final_h0[~infinite_mask]
    infinite_features = final_h0[infinite_mask]
    
    if len(finite_features) > 0:
        finite_persistences = finite_features[:, 1] - finite_features[:, 0]
        print(f"DEBUG: Finite persistence range: {np.min(finite_persistences):.6f} to {np.max(finite_persistences):.6f}")
        print(f"DEBUG: Finite birth range: {np.min(finite_features[:, 0]):.6f} to {np.max(finite_features[:, 0]):.6f}")
        print(f"DEBUG: Finite death range: {np.min(finite_features[:, 1]):.6f} to {np.max(finite_features[:, 1]):.6f}")
    
    if len(infinite_features) > 0:
        print(f"DEBUG: Infinite birth range: {np.min(infinite_features[:, 0]):.6f} to {np.max(infinite_features[:, 0]):.6f}")
        print(f"DEBUG: All infinite deaths are: {set(infinite_features[:, 1])}")
    
    # Check if all samples have same counts
    all_h0_counts = [diag['finite_h0_count'] + diag['infinite_h0_count'] for diag in all_diagrams]
    all_finite_counts = [diag['finite_h0_count'] for diag in all_diagrams]
    all_infinite_counts = [diag['infinite_h0_count'] for diag in all_diagrams]
    
    print(f"DEBUG: H0 total counts across samples: {all_h0_counts}")
    print(f"DEBUG: H0 finite counts across samples: {all_finite_counts}")  
    print(f"DEBUG: H0 infinite counts across samples: {all_infinite_counts}")
    
    if len(set(all_h0_counts)) == 1:
        print(f"DEBUG: ALL SAMPLES HAVE IDENTICAL H0 FEATURE COUNT: {all_h0_counts[0]}")
    else:
        print(f"DEBUG: H0 feature counts vary across samples")
    
    print("DEBUG: End of persistence analysis\n")
...
class PersistenceDiagramCollector:
    ...
    def __init__(self, embedding_space='sbert_concat', distance_metric='euclidean', 
                 bert_data_path=None, device='cuda' if torch.cuda.is_available() else 'cpu'):
...
def main():
    ...
    # Load data
    SAVE_PATH = 'entailment_surfaces/supervised_contrastive_autoencoder/src/persistence_diagrams/collected_diagrams_CONCAT_EUCLIDEAN_DEBUG.pkl'
